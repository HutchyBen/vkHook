...
VkDevice GetLogicalDevice(VkPhysicalDevice pDevice, uint32_t queueFamily) {
    ...
    auto err = vkEnumerateDeviceExtensionProperties(pDevice, nullptr, &properties_count, properties.data());
	if (err != VK_SUCCESS) {
...
VkResult VKAPI_CALL queuePresentHook(VkQueue queue, const VkPresentInfoKHR* pPresentInfo) {
	asm("int3");
...
void main_thread() {
	...
	auto oQueuePresent = (PFN_vkQueuePresentKHR)vkGetDeviceProcAddr(device, "vkQueuePresentKHR");
	
	auto funchook = funchook_create();	
	auto rv = funchook_prepare(funchook, (void**)&oQueuePresent, (void*)queuePresentHook);
	if (rv != 0) {
